...
export default function ChecklistTemplatesList() {
  ...
  const { data: templates = [], isLoading } = trpc.checklistsV2.templates.list.useQuery(
    search ? { search } : undefined
  );

  const deleteMutation = trpc.checklistsV2.templates.delete.useMutation({
    onSuccess: () => {
      toast.success("Modelo de checklist removido!");
        ...
        {isLoading ? (
          ...
          <Card className="bg-card border-border">
            <CardContent className="py-16 text-center">
              ...
              <Link href="/checklists/modelos/novo">
                <Button size="sm" className="mt-4 bg-primary text-primary-foreground">Criar primeiro modelo</Button>
              </Link>
            </CardContent>
          </Card>
        ) : (
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-border bg-secondary/30">
                  ...
                  <th className="text-left text-xs font-semibold text-muted-foreground uppercase px-4 py-3 hidden md:table-cell">Recorrência</th>
              ...
              <tbody className="divide-y divide-border">
                {templates.map((ct: any) => (
                  <tr key={ct.id} className="hover:bg-secondary/20 transition-colors">
                    ...
                    <td className="px-4 py-3 hidden lg:table-cell">
                      <span className="text-sm text-muted-foreground">
                        {new Date(ct.createdAt).toLocaleDateString("pt-BR")}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-1">
                        <Link href={`/checklists/modelos/${ct.id}/editar`}>
                          ...
                          <AlertDialogContent className="bg-card border-border">
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription>
                                O modelo <strong>{ct.name}</strong> será removido permanentemente.
                              </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
                              ...
                              <AlertDialogAction
                                ...
                                onClick={() => deleteMutation.mutate({ id: ct.id })}
